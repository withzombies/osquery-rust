...
use osquery_rust_ng::plugin::{
    ColumnDef, ColumnOptions, ColumnType, Plugin, ReadOnlyTable,
};
use osquery_rust_ng::{
    ExtensionPluginRequest, ExtensionResponse, ExtensionStatus, Server,
};
...
fn test_multi_plugin_coordination() {
    ...
    stop_handle.stop();
    server_handle.join().expect("Multi-plugin server should complete");
    mock_handle.join().expect("Mock osquery should complete");
...
fn test_plugin_error_resilience() {
    ...
    stop_handle.stop();
    server_handle.join().expect("Error resilience server should complete");
    mock_handle.join().expect("Mock osquery should complete");
...
fn test_resource_cleanup_on_shutdown() {
    ...
    server_handle.join().expect("Cleanup server should complete gracefully");
    mock_handle.join().expect("Mock osquery should complete");
    
    // Verify socket was cleaned up
    assert!(!socket_path.exists() || socket_path.to_str().unwrap().contains("cleanup_test"), 
            "Extension socket should be cleaned up or properly named");
