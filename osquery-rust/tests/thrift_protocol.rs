...
use osquery_rust_ng::{ThriftClient, OsqueryClient};
...
fn spawn_malformed_mock(socket_path: &std::path::Path, response_type: &str) -> thread::JoinHandle<()> {
...
fn spawn_connection_dropping_mock(socket_path: &std::path::Path) -> thread::JoinHandle<()> {
    ...
    thread::spawn(move || {
        let listener = UnixListener::bind(&socket_path).expect("Failed to bind connection dropping mock");
...
fn test_empty_response_handling() {
    ...
    assert!(query_result.is_err(), "Query should fail with empty response");
...
fn test_truncated_response_handling() {
    ...
    let ping_result = client.ping();
    assert!(ping_result.is_err(), "Ping should fail with truncated response");
...
fn test_connection_drop_handling() {
    ...
    let ping_result = client.ping();
    assert!(ping_result.is_err(), "Ping should fail when connection drops");
...
fn test_slow_response_handling() {
    ...
    assert!(ping_result.is_err(), "Ping should timeout with slow response");
    assert!(elapsed < Duration::from_secs(2), "Should timeout quickly, not hang");
...
fn test_invalid_protocol_handling() {
    ...
    let ping_result = client.ping();
    assert!(ping_result.is_err(), "Ping should fail with invalid protocol");
...
fn test_concurrent_client_connections() {
    ...
    let mock_handle = thread::spawn(move || {
        let listener = UnixListener::bind(&socket_path_clone).expect("Failed to bind concurrent mock");
        
        for _ in 0..3 { // Handle up to 3 connections
            if let Ok((mut stream, _)) = listener.accept() {
                thread::spawn(move || {
                    let mut buffer = vec![0; 4096];
                    let _ = stream.read(&mut buffer);
    ...
    eprintln!("Concurrent connections test completed: {}/{} successful", 
              successful_connections, results.len());
}
...
fn test_large_request_handling() {
    ...
    let mock_handle = thread::spawn(move || {
        let listener = UnixListener::bind(&socket_path_clone).expect("Failed to bind large request mock");
